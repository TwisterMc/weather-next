...
export default function WeatherDetails() {
    ...
    const displayTemperature = unitSystem === 'imperial'
        ? weather.temperature ?? '--'
        : weather.temperatureC != null
            ? (typeof weather.temperatureC === 'string' ? Math.round(parseFloat(weather.temperatureC)) : Math.round(weather.temperatureC))
            : '--';
    const displayTempUnit = unitSystem === 'imperial' ? 'Fahrenheit' : 'Celsius';
    const displayTempColor = unitSystem === 'imperial'
        ? getTemperatureColor(weather.temperature)
        : getTemperatureColor(typeof weather.temperatureC === 'string' ? parseFloat(weather.temperatureC) : weather.temperatureC, true);
        ...
        <div>
            ...
            <div className={styles.grid}>
                ...
                <div className={`${styles.tile} ${styles.temperatureTile}`}>
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512" className={styles.metricIcon} style={{ fill: displayTempColor }}>
